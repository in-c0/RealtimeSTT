...
if __name__ == '__main__':
    ...
    toxicity_queue = queue.Queue()

    def detect_toxicity_in_background(chunks):
        try:
            predictions = detoxify_model.predict([" ".join(chunks)])  # Process in batches
            toxicity_queue.put(predictions["toxicity"])
        except Exception as e:
            console.print(f"[red bold]Error in background toxicity detection: {e}[/red bold]")
    ...
    def text_detected(text):
        ...
        for chunk in chunks:
            try:
                predictions = detoxify_model.predict(chunk)  # Returns a dictionary of scores
                is_toxic = predictions["toxicity"] > 0.7  # Use the specific "toxicity" key
    ...
    recorder_config = {
        ...
        'realtime_model_type': 'base.en', # or small.en or distil-small.en or tiny.en or small.en
        'language': 'en',
